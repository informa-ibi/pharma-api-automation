...
const { deleteWildcardSearchingSymbol } = require("../../util/commonUtils");
const { SEARCH_ENTITY_ENDPOINT } = require("../../constants/apiEndpoints");
...
When(/^Send a POST request to trial entity with '(.*)' search body$/, async (conditional) => {
  const requestUrl = SEARCH_ENTITY_ENDPOINT("trial");
  this.postSearchData = trialSearchData.searchBody[conditional];
  const response = await $firstCommonAPIClient.postRequest(requestUrl, this.postSearchData);
...
When(/^Send a POST request to drug entity with '(.*)' search body$/, async (conditional) => {
  const requestUrl = SEARCH_ENTITY_ENDPOINT("drug");
...
When(/^Send a POST request to organization entity with '(.*)' search body$/, async (conditional) => {
  const requestUrl = SEARCH_ENTITY_ENDPOINT("organization ");
